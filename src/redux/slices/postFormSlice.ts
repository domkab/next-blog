...
import { FormData, ImageData, ImageMeta } from "@/types/FormData";
...
const postFormSlice = createSlice({
  ...
  reducers: {
    ...
    removeInlineImage: (state, action: PayloadAction<string>) => {
      state.images.inline = state.images.inline.filter(
        img => img.id !== action.payload,
      );
    },
    updateMainImageMeta: (
      state,
      action: PayloadAction<{ meta: Partial<ImageMeta> }>,
    ) => {
